...
const ChargeModal = ({ isOpen, onClose, onConfirm, currentBalance, requiredAmount }) => {
  ...
  const suggestedAmount = Math.max(insufficientAmount, 10000); // 최소 10,000 CM
    ...
    <div className="charge-modal-overlay">
      <div className="charge-modal-container">
        <div className="charge-modal-header">
          <h2 className="charge-modal-title">CM 충전</h2>
          <button 
            type="button" 
            onClick={onClose}
        ...
        <div className="charge-modal-body">
          ...
          <div className="charge-modal-info">
            <div className="charge-modal-info-item">
              <span className="charge-modal-info-label">현재 보유 CM:</span>
              <span className="charge-modal-info-value">{currentBalance.toLocaleString()} CM</span>
            </div>
            <div className="charge-modal-info-item">
              <span className="charge-modal-info-label">필요한 CM:</span>
              <span className="charge-modal-info-value">{requiredAmount.toLocaleString()} CM</span>
            </div>
            <div className="charge-modal-info-item charge-modal-insufficient">
              <span className="charge-modal-info-label">부족한 CM:</span>
              <span className="charge-modal-info-value charge-modal-insufficient-value">
                {insufficientAmount.toLocaleString()} CM
              </span>
            </div>
          </div>
          ...
          <div className="charge-modal-form-group">
            <label className="charge-modal-label">충전 금액 (CM)</label>
            <input
              type="number"
              value={chargeAmount}
          ...
          <div className="charge-modal-quick-charge">
            ...
            <div className="charge-modal-quick-buttons">
              ...
              >
                {suggestedAmount.toLocaleString()} CM
              </button>
              <button
                type="button"
          ...
          {chargeAmount && parseInt(chargeAmount) > 0 && (
            <div className="charge-modal-balance-preview">
              ...
              <span className="charge-modal-balance-value">
                {(currentBalance + parseInt(chargeAmount)).toLocaleString()} CM
              </span>
            </div>
          )}
        </div>
