...
export const storeCategoryFilter = () => {
  const token = localStorage.getItem("access-token"); // 항상 최신 토큰
  return api.get("http://localhost:19091/api/user/storeList", {
    headers: {
      Authorization: `${token}`
    }
  });
...
export const storeList = (user_index, store_category_index) => {
  const token = localStorage.getItem("access-token"); // 항상 최신 토큰
  return api.get("http://localhost:19091/api/user/storeList/filtered", {
...
export const storeDetail = (store_index) => {
  const token = localStorage.getItem("access-token"); // 항상 최신 토큰
  return api.get("http://localhost:19091/api/user/storeList/detail", {
    params: {store_index},
    headers: {
      Authorization: `${token}`
    }
